...
//   ROYAL_MAIL_SERVICE_CODE        -> RM service code to assign (default "CRL1" = Royal Mail 1st Class;
//                                     use a tracked code like "TPN24" if you need a tracking number)
//   ROYAL_MAIL_PACKAGE_FORMAT      -> "smallParcel" | "mediumParcel" | "largeLetter" | "letter" (default "largeLetter")
 ...
 * @param {Object} args.customer  - { email, account: { firstName, lastName, phoneNumber, address, deliveryAddress, deliveryZipCode } }
...
export async function createRoyalMailOrder({ order, customer }) {
  ...
  const postcode = account.deliveryZipCode || "";

  const weightInGrams = Number(process.env.ROYAL_MAIL_WEIGHT_GRAMS) || 100;
  // Large Letter is the package format; the service (1st Class etc.) is the service code.
  // serviceCode is ACCOUNT-SPECIFIC: it must exist in your Click & Drop Service Register.
  // Leave ROYAL_MAIL_SERVICE_CODE unset to create the order without a service (order still
  // created; assign/label it via your Click & Drop account default), or set it to your
  // account's exact 1st Class Large Letter code from Click & Drop > Settings > Postage services.
  const packageFormatIdentifier =
    process.env.ROYAL_MAIL_PACKAGE_FORMAT || "largeLetter";
  const serviceCode = process.env.ROYAL_MAIL_SERVICE_CODE; // optional, account-specific
